...
fda<-function(x,shape,scale,thres){
  prop = 1-(exp(-(((x-thres)/scale)^(shape))))
  prop
}
...
shinyServer(function(input,output){
  ...
  prop <- reactive({
    fda(selectedData1()$claseB, shapeReact(), scaleReact(), 7.2)-fda(selectedData1()$claseA, shapeReact(), scaleReact(),7.2)
  })
  dataff<-reactive({
    data.frame(clases = selectedData1()$clases, prop = prop(), naha = prop()*input$psize)
  })
  ...
  tableOne<-reactive({
    data.frame(Class = selectedData1()$clases, Naha = round(prop()*input$psize))
  })
